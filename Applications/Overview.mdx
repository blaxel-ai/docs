...
description: 'Deploy long-running services as publicly accessible endpoints with revision management and custom URLs.'
---
...
Applications run on Blaxel's mk3 micro-VM infrastructure and support revision management, custom domain URLs, and deployment from source code, images, or forked sandboxes.
...
You can turn a running sandbox into an application by forking it. This produces a **proxy application** that rebinds to the forked sandbox — the application has no compute of its own to build. Its `image`, `memory`, `envs`, and `port` are inherited from the source sandbox, so the fork starts serving the sandbox's current state on a stable application URL with revision management and custom domains on top.
