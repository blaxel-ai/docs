...
Applications are long-running deployed services that run your custom code as publicly accessible endpoints. Unlike agents (which are request-scoped), applications stay running continuously and are ideal for web servers, APIs, dashboards, and any service that needs to be always-on.

Applications run on Blaxel's mk3 infrastructure and support revision management with canary traffic splitting, custom domain URLs, and deployment from source code, container images, or forked sandboxes.

<Note>
Application Runtime is gated by the `applications` feature flag. Contact Blaxel support to enable it on your workspace.
</Note>
...
To deploy a pre-built container image, set the `image` field:
...
Each deploy creates a new revision. A maximum of 5 revisions are kept per application. Revisions contain the container image, environment variables, memory allocation, and port configuration.
...
<Note>
The domain must be a verified custom domain in your workspace. Custom domain verification is managed in the [Administration section](/Administration/Custom-domains) of the Blaxel Console.
</Note>
