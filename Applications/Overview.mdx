...
const snapshot = await sandbox.snapshots.create("release-candidate");
...
# 1. Snapshot the sandbox while it is in the state you want to serve.
snapshot = await sandbox.snapshots.create("release-candidate")
...
List your existing snapshots with `sandbox.snapshots.list()`. See [Snapshots and forking](/Sandboxes/Fork) for the full snapshot lifecycle.
...
For more on sandbox snapshots and forking sandboxes into new sandboxes, see [Sandbox snapshots and forking](/Sandboxes/Fork).
...
<CardGroup cols={2}>
  <Card title="Sandbox snapshots and forking" icon="code-fork" href="/Sandboxes/Fork">
