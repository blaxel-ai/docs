...
You can turn a running sandbox into an application by [forking it](/Sandboxes/Fork). This produces a **proxy application** that rebinds to the forked sandbox — the application has no compute of its own to build. Its `image`, `memory`, `envs`, and `port` are inherited from the source sandbox, so the fork starts serving the sandbox's current state on a stable application URL with revision management and custom domains on top.
...
// Fork the sandbox into a proxy application named "my-app".
...
<Note>
If an application with `targetName` already exists, forking updates its compute so the backend generates a fresh revision proxying to the (forked) sandbox. Otherwise a new application is created. Forking into an application requires the Application Runtime feature to be enabled on your workspace.
</Note>
