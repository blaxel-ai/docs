---
title: Generations
description: "Overview of Blaxel infrastructure generations from Mark 1 through Carbon."
---

Blaxel workloads run on one of several infrastructure generations, described below from most to least recent.

## Carbon (a.k.a. Mark 3.1)

Carbon is the newest Blaxel infrastructure generation. Currently in private preview, it's built directly as an evolution of Mark 3.0: same microVM foundation, same SDK, CLI, and REST surface. It supports many more features than 3.0, including a layer for manually snapshotting and forking sandboxes.

On Mark 3.0, a sandbox is already snapshotted automatically and invisibly whenever it goes into standby, which is what makes its near-instant resume possible. Carbon exposes that same underlying mechanism directly, so you can:
...
Carbon is currently in private preview and is rolling out progressively by region and by workspace, so it isn't yet the default everywhere. A few Mark 3.0 features aren't yet available together with Carbon:
* Agent Drive
* Firewalling
