...
<Tip>
  Because different dimensions use different units, **mixed-unit totals are deliberately omitted**. `summary.totalUsage` (and per-group `summary.usage`) is **only populated when `groupBy=billing_dimension` or `groupBy=resource_type`**. For every other grouping you will only see `totalCost` / `cost` in USD — the unit-safe common denominator.
</Tip>

## Response

A successful call returns `200 OK` with an `AccountMetricsResponse`:
  ...
  "data": [
    {
      ...
      "summary": { "cost": 102.11, "usage": 48210.55 },
      "timeseries": [
        { "timestamp": "2025-10-01T00:00:00Z", "cost": 3.41, "usage": 1620.03 },
        { "timestamp": "2025-10-02T00:00:00Z", "cost": 3.12, "usage": 1498.77 },
        "..."
      ]
    },
    {
      "resourceType": "agent",
      "summary": { "cost": 28.90, "usage": 2910.12 },
      "timeseries": ["..."]
    }
  ],
  "meta": {
...
| `summary.totalUsage` | Total usage across the window. **Only populated** when `groupBy=resource_type` or `groupBy=billing_dimension`, where every row shares a comparable unit. Omitted otherwise. |
...
- `summary.usage` — usage for this group. Only populated for `groupBy=resource_type` and `groupBy=billing_dimension` (same unit-safety rules as the top-level summary).
