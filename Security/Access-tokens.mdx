...
description: 'Authenticate to Blaxel APIs and the CLI using personal access tokens, with guidance on creating, scoping, rotating, and revoking tokens securely.'
...
**OAuth tokens** are recommended for security reasons, as their duration is only 2 hours (short-lived). They are generated through [OAuth 2.0](https://oauth.net/2/) authentication endpoints.

<Note>
  A personal API key or personal OAuth login grants access to every [workspace](/Security/Workspace-access-control) your account belongs to, while a service account's API key or client-credentials token only grants access to the single workspace that service account belongs to. If a request fails with a 403 error even though the credential is valid, this usually means the credential doesn't have access to the specified workspace.
</Note>
...
![api-key-create.webp](/Security/Access-tokens/api-key-create.webp)
...
<Tip>Your [permissions in each workspace](/Security/Workspace-access-control) will be the ones given to your account in each of them.</Tip>
...
![sa-create2.webp](/Security/Access-tokens/sa-create2.webp)

<Info>Service accounts can also connect to the API using a long-lived API key, as detailed [in the section below](/Security/Access-tokens).</Info>
...
Alternatively, you can also pass the *client ID* and *client secret* in the body:
...
You will retrieve a bearer token valid for 2 hours, which can then be passed in any call to the Blaxel APIs through any of the following headers: `Authorization` or `X-Blaxel-Authorization`, as such:
...
<Note>
  **Common errors**: a missing or incorrect `grant_type` value, or an invalid/missing `client_secret`, both return a `400` error describing the specific parameter that's invalid or missing. Double check the *client ID* and *client secret* were copied correctly — the secret is only shown once at creation and can't be retrieved again.
</Note>

### (Advanced) Use OAuth tokens in the APIs

<Note>This section assumes you are a developer experienced with OAuth 2.0. For a simpler guide of how to use short-lived tokens in Blaxel APIs, read the [section on authenticating service accounts](/Security/Access-tokens).</Note>
...
*Note: when using the Blaxel SDK to operate Blaxel (e.g. to create an agent), token retrieval and refresh [are done automatically based on either being authenticated with CLI or using environment variables](../sdk-reference/introduction).*
