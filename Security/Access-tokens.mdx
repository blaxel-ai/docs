...
This solution requires a catch-all route in the client application (e.g. `/api/blaxel`) that serves as a proxy, forwarding all SDK requests to Blaxel's API. The route reads the Blaxel API key securely from a host environment variable (e.g. `BL_API_KEY`) and injects it into each request by adding `Authorization` and `X-Blaxel-Workspace` headers.

Here is an example of this catch-all route implemented in Next.js:
