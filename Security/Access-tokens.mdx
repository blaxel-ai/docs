...
#### Use API keys via proxy

If you wish to call the Blaxel API directly from a client Web application, you would have to include your Blaxel API API key in client-side code. This creates a serious security vulnerability, as the key will become visible in network requests or in the source code on the client end of the connection.

The solution is to add a proxy between your client application and the Blaxel API. The proxy serves as a secure intermediary layer, which intercepts requests from the client application, injects your Blaxel API key, and then forwards them to Blaxel.

This solution requires a catch-all route in the client application (e.g. `/api/blaxel`) that serves as a proxy, forwarding all SDK requests to Blaxel's API. The route reads the Blaxel workspace and API key securely from host environment variables (e.g. `BL_WORKSPACE` and `BL_API_KEY`) and injects it into each request by adding `Authorization` and `X-Blaxel-Workspace` headers.
...
async function handleRequest(request: NextRequest, { params }: { params: { slug: string[] } }) {
  ...
  const response = await fetch(`https://run.blaxel.ai/${path}`, fetchOptions);
...
<Note>
The above code is illustrative and not intended for production use. It has no authentication, allowing access to anyone who knows the endpoint URL. In production, additional authentication should always be added to the route to prevent unauthorized access.
</Note>

Now, within the application, the Blaxel SDK should be configured at init-time to use the proxy route, as shown below. With this change, SDK requests are routed via the named proxy rather than directly transmitted to the Blaxel API.

```typescript
import blaxel from "@blaxel/core";

blaxel.initialize({
  proxy: 'http://localhost:3000/api/blaxel',  // Your proxy endpoint
...
Under this approach, your Blaxel API key is never exposed to the calling client, ensuring security. SDK requests continue to work as before, except that now they are automatically routed via the proxy rather than directly to the Blaxel API.
