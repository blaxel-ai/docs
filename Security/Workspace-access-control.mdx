...
All resources on Blaxel are logically regrouped in a **workspace,** which is the highest possible level of tenancy. Your organization will usually operate within one single workspace, but it is not necessarily the case when dealing with multiple business units or end-clients for example.

Users can be either [team members](Workspace-access-control%2013847e47b1ea8151bd43efeccf5defe0), or [service accounts](Service-accounts) that represent external systems that can operate Blaxel. They are added to a workspace with certain permissions on the workspace resources inherited from their role.
