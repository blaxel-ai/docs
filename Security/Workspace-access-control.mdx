...
Users can be either [team members](Workspace-access-control.mdx), or [service accounts](Service-accounts.mdx) that represent external systems that can operate Beamlit (typically, a [Beamlit operator](../Models/Cloud-Burst-Network/Install-Beamlit-operator.mdx)). They are added to a workspace with certain permissions on the workspace resources inherited from their role.
...
- [models](../Models/Model-deployment.mdx) (including [querying](../Models/Query-a-model.mdx) them)
- [model deployments](../Models/Model-deployment.mdx)
