...
Run a command inside a sandbox on a recurring cron, at a future date, or after a delay. Each schedule fires through the sandbox process API and records its outcome in an execution history. See [Schedules](/Sandboxes/Schedules).
