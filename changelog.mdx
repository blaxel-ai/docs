...
Run OpenAI-hosted agents on disposable Blaxel Sandboxes with the [Agents API cookbook](https://github.com/blaxel-ai/blaxel-openai-agents-api-cookbook). A Blaxel-hosted webhook handler starts or reconnects the Sandbox when OpenAI asks for one, only a restricted executor key enters the Sandbox, and Agent Drive carries files to a replacement Sandbox. See the [tutorial](/Tutorials/OpenAI-Agents-API).
