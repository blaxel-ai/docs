...
Every Agent Drive instance now exposes an [S3-compatible endpoint](/Agent-drive/Overview#access-a-drive-over-s3), so you can access its contents with the AWS CLI, boto3, or the AWS SDK for JavaScript without mounting the drive into a sandbox.
...
### Ephemeral root storage for sandboxes

In private preview, sandboxes can now provision [disk-backed root storage](/Sandboxes/Overview#memory-and-filesystem) with `storageMb`, adding filesystem capacity without increasing memory. The storage follows the sandbox lifecycle and is deleted with the sandbox.
...
Verified preview custom domains can now be [shared with another workspace or across an account](/Infrastructure/Custom-domains#share-a-custom-domain). Shared domains remain read-only in target workspaces and can be used for their sandbox previews.
