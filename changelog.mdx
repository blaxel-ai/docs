...
### Snapshots outlive their sandbox

[Snapshots](/Snapshots/Overview) are now workspace resources with their own name and API. Deleting a sandbox no longer deletes its snapshots, and a snapshot carries the configuration needed to be forked into a new sandbox or application once its source is gone.
