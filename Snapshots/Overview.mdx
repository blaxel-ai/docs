...
| `id` | Internal identifier of the stored snapshot. Read-only, and accepted wherever a snapshot name is expected |
| `source` | Object the snapshot was captured from: `kind` (`sandbox`), `name`, and `deleted` |
...
A snapshot always has a source. Pass the object to capture as `source`, and a `name` if you want to choose it:

<CodeGroup>

```typescript TypeScript
import { SnapshotInstance } from "@blaxel/core";

const snapshot = await SnapshotInstance.create({
...
```python Python
from blaxel.core import SnapshotInstance

snapshot = await SnapshotInstance.create({
    "name": "before-migration",
    "source": {"kind": "sandbox", "name": "my-sandbox"},
})
```
...
<Note>
  `kind` is `sandbox` today. It exists so other kinds of sources can be captured later without changing the shape of a snapshot.
</Note>
...
const snapshot = await SnapshotInstance.get("before-migration");
...
snapshot = await SnapshotInstance.get("before-migration")
print(snapshot.source.name, snapshot.source.deleted)
```

```bash HTTP API
curl https://api.blaxel.ai/v0/snapshots/before-migration \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace"
```
...
const snapshots = await SnapshotInstance.list({ limit: 50 });
...
async def main():
    snapshots = await SnapshotInstance.list(limit=50)
...
```bash
curl 'https://api.blaxel.ai/v0/snapshots?limit=50&q=migration' \
...
```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/snapshots/before-migration/fork \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace" \
  -H "Content-Type: application/json" \
  -d '{
...
await SnapshotInstance.delete("before-migration");
```
...
```bash HTTP API
curl -X DELETE https://api.blaxel.ai/v0/snapshots/before-migration \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace"
```

```shell Blaxel CLI
bl delete snapshot before-migration
```
