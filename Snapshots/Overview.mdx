...
| Field | Description |
|---|---|
| `name` | Workspace-unique name you address the snapshot by. Blaxel generates a UUID-style name when you do not pass one |
...
<Note>
  Environment variables are deliberately not part of the snapshot spec. The values the captured processes were running with are already inside the memory image, so Blaxel does not copy them onto the snapshot. A fork from a snapshot whose source sandbox is deleted therefore starts with no environment variable configuration of its own. Pass `envs` to set the variables the new sandbox or application declares.
</Note>
