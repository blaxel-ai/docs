---
title: "Connect Claude Agent SDK to sandboxes"
description: "Connect the Claude Agent SDK (formerly Claude Code SDK) to a Blaxel sandbox to operate processes, files, and code through its built-in MCP server."
sidebarTitle: "Connect from Claude Agent SDK"
---
...
- The agent configuration includes the sandbox MCP server URL and uses the Blaxel API key as a credential to gain access to it (the `Authorization` header).
