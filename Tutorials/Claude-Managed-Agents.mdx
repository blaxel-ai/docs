---
title: 'Deploy a self-hosted Claude Managed Agents on Blaxel'
description: 'Deploy a self-hosted Claude Managed Agents environment on Blaxel for scalable, observable agent execution.'
---

Blaxel integrates with [Claude Managed Agents (CMA)](https://platform.claude.com/docs/en/managed-agents/overview), allowing you to self-host the sandbox layer while keeping the agent loop entirely within Anthropic's platform. Through this integration, agents have direct access to create and manage Blaxel [sandboxes](https://docs.blaxel.ai/Sandboxes/Overview), enabling them to call tools, execute commands, work with files and directories, write code, and perform computations in fully isolated environments.
...
- The [Anthropic CLI] (optional). [Download and install the Anthropic CLI](https://platform.claude.com/docs/en/api/sdks/cli).
...
![CMA flow diagram](/img/tutorials/cma/cma-flow.png)
...
- The worker sandbox persists until the session ends, at which point it exits and is auto-deleted after its TTL expires. The orchestrator sandbox is a long-running sandbox which continues to exist to receive future events.

## Clone the repository and log in to Blaxel
...
export BL_WORKSPACE=<Blaxel-workspace-id>
...
## Build and push the sandbox images
...
- Set the endpoint URL to the preview URL from the previous step and subscribe to all session lifecycle events.
...
When the session starts, the Claude Platform sends a `session.status_run_started` event to the orchestrator. The orchestrator schedules a background task that creates the worker sandbox, claims the work item from Anthropic's queue, and uses the sandbox to execute tool calls and post results back to Anthropic. The worker exits when idle and is auto-deleted after its TTL expires.
