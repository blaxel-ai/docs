...
This creates a Next.js coding agent that accepts requests from the command-line. The coding agent's sandbox is generated from Blaxel's `blaxel/nextjs` image, which contains a complete Next.js development environment and a Next.js skeleton application.
...
```shell
python main.py "Create a website about Kitty, the cat that does amazing things"
```

The agent will start working on the task in a sandbox. Once it is complete, you should see output similar to the following:
