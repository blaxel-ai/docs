...
description: 'Run OpenClaw on Blaxel using an agent template'
---
...
Blaxel's agent template uses autoscaling to keep at least one OpenClaw instance running at all times. Autoscaling is only available on Tier 2 and higher plans.
...
1. Update the `.env` file with the following configuration values:
    ...
    - `OPENCLAW_MODEL` (optional): Set the model to use in `provider/model` format. Defaults to `anthropic/claude-opus-4-6` if not set.
    - `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` or `OPENAI_API_KEY`: Set the model API key matching your chosen model provider.
    ...
    <Tip>
      You can also set `BL_REGION` to choose the deployment region (defaults to `us-pdx-1`). This controls where both the agent and its persistent storage volume are provisioned.
    </Tip>

### Persistent storage

The template includes a 2 GB persistent volume mounted at `/root/.openclaw`. This volume preserves OpenClaw's data (conversation history, configuration, and agent state) across restarts and redeployments.

The volume is automatically provisioned in the same region as the agent when you run `bl deploy`. No additional configuration is required.
