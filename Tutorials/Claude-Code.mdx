...
- An Anthropic API key, required by Claude Code. If not, [sign up for an Anthropic account](https://platform.claude.com/) and obtain an API key.
- A Blaxel account. If not, [sign up for a Blaxel account](https://blaxel.ai).
- A Blaxel API key, required by Claude Code to connect to the codebase sandbox using Model Context Protocol (MCP). If not, o
