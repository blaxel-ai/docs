...
description: 'Run Claude Code inside a Blaxel sandbox to execute coding tasks on a hosted codebase, with persistent storage and secure network access.'
---
...
1. Create a script named `main.py` (Python) or `index.ts` (TypeScript) in the same directory.
    ...
    async function main() {
      ...
      const claudeSandbox = await SandboxInstance.createIfNotExists({
        name: "claude-sandbox",
        image: "blaxel/claude-code:latest",
        memory: 4096,
        region: "us-pdx-1",
        envs: [
...
1. Confirm Claude Code is available in the sandbox:
