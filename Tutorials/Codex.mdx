...
1. Create a script named `main.py` (Python) or `index.ts` (TypeScript) in the same directory.
    ...
    - adds the OpenAI API key to `codex-sandbox` as an environment variable named `OPENAI_API_KEY`.
...
1. Start Codex, passing the application sandbox's MCP server URL (obtained from the sandbox creation script in the previous section) and your Blaxel API key:

    ```shell
    codex --mcp-server '{"type":"http","url":"YOUR-SANDBOX-MCP-URL-HERE","headers":{"Authorization":"Bearer YOUR-BLAXEL-API-KEY-HERE"}}'
    ```

Codex will connect to the application sandbox's MCP server and prompt you for a task.
...
<CardGroup>
  <Card title="OpenAI Agents SDK" icon="star-christmas" href="/Tutorials/OpenAI-Agents-SDK">
    Build and deploy OpenAI Agents SDK agents on Blaxel.
  </Card>
