...
description: 'Deploy MCP servers on Blaxel as serverless, autoscalable APIs to equip your AI agents with tools, integrations, and external system access.'
...
Blaxel SDK lets you **retrieve the tools from an MCP server in your code**. When both an agent and MCP server run on Blaxel, the tool call in the MCP server is executed separately from the agent logic. This ensures not only optimal resource utilization, but also better design practice for your agentic system.
