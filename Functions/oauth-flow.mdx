...
description: 'Set up a user-facing OAuth flow so end-users can authorize integration connections to third-party services through your Blaxel application.'

---

<Warning>
  This page covers end-user OAuth: letting your **end users** authorize third-party services (e.g. Gmail) so their integration can be bound to an MCP server you deploy. It is **not** about authenticating a client to a Blaxel MCP endpoint. MCP servers that you deploy on Blaxel authenticate clients with bearer tokens. To sign a client in to the hosted Blaxel MCP server with OAuth, see [Blaxel Skills & MCP](/skills-mcp).
</Warning>
...
- Choose a name. It will be displayed when your user logs in through Google
...
Finish creating the application. You’ll be given a *client_id* and *client_secret:* keep them securely.

### 2. Configure scope

Scopes are used to request sufficient access on the user’s account. In this example, we want to connect their account to the MCP server to send emails.
...
<Info>By default, it will work with your own account without any review by Google. You can look for the Audience tab on the left to add more users. To make it global, you will need to launch a review process from Google with the “Publish app” button.</Info>
...
- Google prompts user to log in and approve requested scopes
...
```bash
// Create the MCP bound to the integration connection
