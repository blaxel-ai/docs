...
description: 'Host custom MCP servers on Blaxel as serverless, autoscalable endpoints with the CLI, GitHub integration, or Dockerfile-based deployments.'
...
This section assumes you have developed the MCP server locally, as explained [in this documentation](/Functions/Create-MCP-server), and are ready to deploy it.
...
![image.webp](/Functions/Deploy-a-function/image.webp)
...
![image.webp](/Agents/Deploy-an-agent/image1.webp)
...
Blaxel supports ["code mode"](/Functions/Code-mode) natively, enabling a more efficient way to execute tool calls over MCP.
...
This `entrypoint` section is optional. If not specified, Blaxel will automatically detect the MCP server’s content and configure your server’s startup settings.
...
- `scripts.prod` : start the server remotely on Blaxel from the dist folder, the project needs to have been built before.
