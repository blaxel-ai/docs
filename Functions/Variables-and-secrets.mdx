...
title: 'Variables and secrets for MCP servers'
sidebarTitle: 'Variables and secrets'

description: 'Configure environment variables and secrets for Blaxel MCP servers to securely store API keys, credentials, and runtime configuration values.'
...
This lets you specify a different environment file for production secrets instead of relying on the default *.env*, e.g.:
...
Non-secret build variables can be placed in the `blaxel.toml` file, as in the example below:
...
`PORT` : Port of the HTTP server, it needs to be set to allow Blaxel platform to configure it

`HOST` : Host of the HTTP server, it needs to be set to allow Blaxel platform to configure it

Internal URL for Blaxel platform, to avoid linking multiple instances through the Internet
