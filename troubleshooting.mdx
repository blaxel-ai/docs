...
This error occurs when the application server running inside the sandbox is binding to `localhost`. As a result, it is only reachable inside the sandbox; the external preview URL will not be able to access it.

To resolve this error, configure your application server to bind to IP address `0.0.0.0`, so that it listens on all available network interfaces, and then restart the application server. To avoid IPv4/IPv6 compatibility issues, we recommend using the environment variable `HOST` instead of setting the address to a static value.
...
To resolve this error, delete and recreate the sandbox, which also restarts the sandbox API.
