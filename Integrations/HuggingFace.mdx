...
The integration must be set up by an [admin](../Security/Workspace-access-control.mdx) in the Integrations section in the [workspace settings](../Security/Workspace-access-control.mdx).
...
Once you’ve [set up](HuggingFace.mdx) the integration in the workspace, any workspace member can use it to reference a HuggingFace repository as the origin for a [model deployment](../Models/Model-deployment.mdx). 

### Public and private models

When [deploying a model](../Models/Model-deployment.mdx), select “Deploy from HuggingFace”. You can search for any **public model**, or any **private model** in the organizations & repositories allowed by the integration’s token.
