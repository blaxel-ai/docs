...
- Agent Drives: [Agent Drives](/Agent-drive/Overview) behave like a shared, distributed filesystem that can be mounted to multiple sandboxes or agents at any time. They are highly optimized for small and medium-sized file operations.

  Agent Drives use a fine-grained replication strategy and are also separately backed up. They scale automatically with no fixed capacity limits. Pre-provisioning or run-time resizing is not required.
