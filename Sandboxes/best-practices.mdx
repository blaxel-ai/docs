...
Choosing the right storage for your sandbox data is important, as it can impact your agent's performance, reliability, and overall consistency. Blaxel offers four options:
...
- Ephemeral volumes: [Ephemeral volumes](/Volumes/Ephemeral-volumes) are disks created with the sandbox and destroyed with it. They extend the writable disk space of a sandbox beyond the default filesystem, with a size you choose at creation. Use them for large scratch data such as build outputs, package caches, or datasets that do not need to outlive the sandbox.
...
|   | Sandbox Filesystem | Ephemeral Volume |  Volume |  Agent Drive |
|---|---|---|---|---|
| Persistence | Until sandbox deletion | Until sandbox deletion | Survives sandbox deletion | Survives sandbox deletion |
| Durability | Low (lost on sandbox deletion or crash) | Low (lost on sandbox deletion or crash) | High (block storage, replicated) | High (distributed storage, replicated) |
| Access type | Single | Single | Single | Multiple |
| Hot-attach | N/A | No | No | Yes |
| Interface | POSIX | POSIX | POSIX | POSIX + S3 |
| Size | ~50% of sandbox memory by default; configurable with root storage | User-defined at creation, several per sandbox | User-defined at creation, expandable | No size limit |
| Optimized for | Data that doesn’t require high durability | Large scratch data that doesn't need durability | Data that requires durability with high performance | Shared data |
...
<Note>
  There is no built-in way to bulk-sync files between a sandbox and your local machine. Instead, use [`sandbox.fs.download()`](/Sandboxes/Filesystem#download-file-to-host) for single files, or the zip-upload-unzip pattern in the next section for bulk transfers.
</Note>
...
Combine external IDs with [TTL policies](/Sandboxes/Expiration) and [labels](/Sandboxes/Overview#labels) for a complete sandbox management strategy: external IDs for fast lookup, labels for categorization, and TTLs for automatic cleanup.
