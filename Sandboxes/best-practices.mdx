...
When you manage many sandboxes per workspace (hundreds or thousands), retrieving the right one by name alone becomes impractical. You often need to find a sandbox by a caller-owned identifier: a session ID, user ID, or conversation ID from your application. Requires `@blaxel/core` ≥ 0.2.92 or `blaxel` ≥ 0.2.57.
