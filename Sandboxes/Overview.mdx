...
```bash
(cd my-custom-sandbox && zip -r ../my-sandbox.zip .)
```
...
const SOURCE_DIR = "my-sandbox";
const ZIP_FILE = "my-sandbox.zip";
const BASE_URL = "https://api.blaxel.ai/v0";

const apiKey = process.env.BL_API_KEY;
const workspace = process.env.BL_WORKSPACE;

if (!apiKey) {
  console.error("Error: BL_API_KEY not set");
  process.exit(1);
}

if (!workspace) {
  console.error("Error: BL_WORKSPACE not set");
  process.exit(1);
}
...
async function main() {
  try {
    ...
    const createResponse = await fetch(`${BASE_URL}/sandboxes?upload=true`, {
      method: "POST",
      headers: {
...
if not api_key:
    print("Error: BL_API_KEY not set")
    sys.exit(1)

if not workspace:
    print("Error: BL_WORKSPACE not set")
    sys.exit(1)
...
Create a sandbox directory named `my-sandbox` with your custom Dockerfile and entrypoint script:
