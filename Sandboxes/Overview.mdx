...
Blaxel sandboxes start from hibernation state in **under 25 milliseconds**, and scale back down to zero **after one second of inactivity**, maintaining the memory state even after scaling down. This includes all filesystem as well as running processes. For cost-effective long-term persistence, you can attach [volumes](Volumes) to sandboxes.
