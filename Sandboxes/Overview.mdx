...
They provide a basic [REST API interface](https://docs.blaxel.ai/api-reference/filesystem) for accessing the file system and processes, along with an [MCP server](/Sandboxes/MCP) that makes these capabilities available as tool calls.
...
<Note>
  To prevent a sandbox from going into standby, you can use WebSocket ping/pong keepalive messages or [other techniques](/Sandboxes/Standby-control) to prevent the timeout.
</Note>

### Deletion

In addition to automatic scale-to-zero, Blaxel also supports [automatic sandbox deletion based on expiration policies](/Sandboxes/Expiration). Starter quotas enforce time-to-live (TTLs), while higher quota tiers unlock sandboxes with unlimited persistence.
...
Blaxel supports [automatic sandbox deletion](/Sandboxes/Expiration) based on specific conditions.
...
Every Blaxel sandbox includes a [custom API binary](/Sandboxes/Templates#how-sandbox-templates-work), which is necessary for sandbox functionality like process management and file operations. It is possible to perform an in-place upgrade of this API without needing to recreate or restart the sandbox.
