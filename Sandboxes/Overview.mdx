...
To retrieve all sandboxes in your workspace, use the Management API list endpoint with pagination or the built-in SDK pagination helper functions.
...
for sandbox in SyncSandboxInstance.list(limit=50).auto_paging_iter():
...
The curl response includes a `data` array and a `meta` object:
...
For more information, refer to the [API](/api-reference/introduction#pagination) and [SDK](/sdk-reference/introduction#pagination) reference documentation.
