...
<Note>This feature is currently in beta and only available for sandboxes created with sandbox API v0.2.0 or later (sandboxes created after 2 Feb 2026). For sandboxes created earlier than this date/API version, in-place upgrade is not possible; the sandbox must be recreated to obtain the new API.</Note>
