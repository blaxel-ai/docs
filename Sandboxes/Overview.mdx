...
Because the sandbox has already been removed, its name is free again and you can safely start the creation over. Catch the error with `isCreationTimeoutError()` and retry with your own policy:
...
for (let attempt = 1; attempt <= 3 && !sandbox; attempt++) {
  try {
    // Fail this attempt if the sandbox is not ready after 30 seconds
    sandbox = await SandboxInstance.createIfNotExists(config, { timeout: 30 });
  } catch (error) {
    if (!isCreationTimeoutError(error)) throw error; // any other error is not retried
    console.warn(`Sandbox creation timed out (attempt ${attempt}), retrying...`);
  }
}
```

`SandboxCreationTimeoutError` exposes `code` (`"CREATION_TIMEOUT"`), `status` (`408`), `sandboxName` and `timeout` so you can log or branch on the failure.
