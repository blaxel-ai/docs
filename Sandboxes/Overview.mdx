...
Due to this, Blaxel sandboxes reserve, when it's possible, approximately 50% of the available memory for the `tmpfs` filesystem. More information on our implementation is available in this [blog post](https://blaxel.ai/blog/how-to-slash-sandbox-memory-usage-by-75-using-overlayfs).

To avoid out-of-memory errors or if additional storage is required, it is always possible to add storage using [volumes](/Sandboxes/Volumes), although these are not as fast as the native in-memory filesystem.
