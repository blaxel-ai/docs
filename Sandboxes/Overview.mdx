...
Managing sandbox hibernation and cost optimization
...
Optimizing sandbox usage to reduce costs

Here are several approaches you can implement to better control when sandboxes remain active:
...
You can use the Sandbox SDK from the frontend if you have the right headers set on the session. Use the sandbox.sessions.create function to manage sessions.

Important limitation: SDK requests are limited to sandbox operations like process, filesystem, and terminal access. You cannot use the SDK to manage sandbox lifecycle (creating, updating, removing sandboxes, or creating volumes) from a session.

Current limitations

Currently, there are no built-in stop or start functions available in the Sandbox SDK for force hibernation. This means you'll need to rely on the connection-based approach and the optimization strategies mentioned above.



An example of auto disconnect on tab switch with NextJS through a plugin
...
(function() {
  ...
  const DISCONNECT_DELAY = ${disconnectDelay};
