...
Standby keeps a sandbox instantly resumable, memory included, and you keep paying for the storage of its snapshot. When a sandbox is not needed for a long time, [archiving](./Archive) it stores the filesystem changes it made over its image and shuts it down: memory and running processes are lost, and nothing is kept warm for it.

An archived sandbox keeps its name, its configuration, and its preview URLs. Unarchiving it starts the sandbox again from its image and writes the archived filesystem back over it, so you get the same sandbox back with its files, and the saved processes start again from their configuration with new IDs.

<Note>
  Archiving differs from standby: standby resumes in milliseconds with memory and processes intact, while unarchiving takes as long as the archived filesystem needs to be written back, and only the filesystem comes back. Archiving is also not a [snapshot](./Fork): a snapshot can be forked into other sandboxes, while an archive only ever restores the sandbox it was taken from.
</Note>
