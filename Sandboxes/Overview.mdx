...
To avoid out-of-memory errors or if additional storage is required, it is always possible to add storage using volumes, although these are not as fast as the native in-memory filesystem.
