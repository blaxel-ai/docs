...
/// Start sandbox creation (exposing port 3000 for example)
...
async def main():
    ...
    sandbox = await SandboxInstance.create(configuration)
    await sandbox.wait()
...
sandbox = await SandboxInstance.create_if_not_exists(configuration)
await sandbox.wait()
