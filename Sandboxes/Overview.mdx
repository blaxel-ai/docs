...
<Warning>SDK requests are limited to sandbox operations like process, filesystem, and terminal access. You cannot use the SDKs to manage sandbox lifecycle (creating, updating, removing sandboxes, or creating volumes) from a session. In addition, there are no built-in stop or start functions available in the SDKs for force hibernation. This means you'll need to rely on the connection-based approach and the optimization strategies mentioned above.</Warning>
