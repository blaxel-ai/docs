...
description: 'Lightweight virtual machines where both you and your agents can run code with sub-25ms cold starts.'
...
Set **time-to-live (TTL)** on the sandbox to **automatically** **delete** it after a specific period using the `ttl` parameter, or at a specific date using the `expires` parameter.
