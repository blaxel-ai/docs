...
`SandboxCreationTimeoutError` exposes `code` (`"CREATION_TIMEOUT"`), `status` (`408`), `sandboxName` and `timeout` so you can log or branch on the failure. The sandbox has already been removed when the error is thrown, so its name is free again if you decide to create it again.

<Note>
  `timeout` must be a whole number between 1 and 50 seconds. It can only shorten the creation deadline, not extend it. This option is currently available in the TypeScript SDK.
</Note>
