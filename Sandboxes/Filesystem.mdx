...
You can write multiple files or directories at once:
...
The `watch` function monitors all file system changes **in the specified directory and its subdirectories.** 
...
You can ignore changes in certain files or directories by providing an array of patterns to ignore:
...
async function testWatchWithIgnore(sandbox: SandboxInstance) {
  ...
  const handle = fs.watch("/", (fileEvent) => {
    ...
    withContent: true,
    ignore: ["app/node_modules", "*.tmp", "test2.txt"]
  });
