...
import { createSandboxSnapshot } from "@blaxel/core";

const { data: snapshot } = await createSandboxSnapshot({
...
from blaxel.core.client.api.compute.create_sandbox_snapshot import asyncio as create_sandbox_snapshot
from blaxel.core.client.models import SandboxSnapshotRequest

snapshot = await create_sandbox_snapshot(
    sandbox_name="my-sandbox",
    client=client,
    body=SandboxSnapshotRequest(name="before-migration"),
)
```
...
```typescript TypeScript
import { listSandboxSnapshots } from "@blaxel/core";

const { data: snapshots } = await listSandboxSnapshots({
  path: { sandboxName: "my-sandbox" },
  throwOnError: true,
});
```
...
```python Python
from blaxel.core.client import client
from blaxel.core.client.api.compute.list_sandbox_snapshots import asyncio as list_sandbox_snapshots

snapshots = await list_sandbox_snapshots(sandbox_name="my-sandbox", client=client)
```
...
```typescript TypeScript
import { forkSandbox } from "@blaxel/core";

const { data: result } = await forkSandbox({
  path: { sandboxName: "my-sandbox" },
  body: { targetName: "my-sandbox-copy", targetType: "sandbox" },
  throwOnError: true,
});
```
...
```python Python
from blaxel.core.client import client
from blaxel.core.client.api.compute.fork_sandbox import asyncio as fork_sandbox
from blaxel.core.client.models import SandboxForkRequest

result = await fork_sandbox(
    sandbox_name="my-sandbox",
    client=client,
    body=SandboxForkRequest(target_name="my-sandbox-copy", target_type="sandbox"),
)
```
