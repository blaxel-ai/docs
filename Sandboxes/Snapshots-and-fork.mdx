...
Use the `snapshotId` parameter to fork from a specific snapshot instead of the current sandbox state. Combine it with `targetType: "sandbox"` to create a new sandbox from a snapshot:
