...
Blaxel supports automatic sandbox deletion based on specific expiration policies. This feature is available in all tiers, although TTL constraints differ. For example, in the "Free" tier, the TTL cannot exceed 7 days whereas, in higher tiers, unlimited TTLs are permitted.

<Note>Blaxel has a quota tiering system that unlocks higher limits and features on the platform as your tier progresses. Higher tiers have access to longer sandbox TTLs. You can see the TTL-related features of each tier in the [Blaxel Console](https://app.blaxel.dev/account/quota).</Note>
