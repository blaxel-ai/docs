...
title: 'Attach volumes to your sandbox'

description: 'Persist files long-term using sandbox volumes.'
...
While Blaxel automatically snapshots the full state of a sandbox at scale down and stores it in a warm storage for ultra-fast boot times, volumes offer a more cost-effective solution to persist files for weeks to years.
...
Any files written to the `/app` directory within this sandbox will be stored on the `nextjs-demo-volume` and will persist even if this sandbox is deleted.
