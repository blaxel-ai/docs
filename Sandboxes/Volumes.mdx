...
<Warning>At this time, you can only attach one persistent volume to a sandbox. A volume can also only be attached to one sandbox at a time.</Warning>
...
Ephemeral volumes are temporary disks created with the sandbox and destroyed when it terminates. They give a sandbox extra writable disk space without creating a volume resource beforehand, and they do not count toward the one-persistent-volume limit: you can attach several ephemeral volumes alongside a persistent one.

Declare an ephemeral volume in the same `volumes` list, with `type` set to `ephemeral` and a `sizeMb`:
...
Data written to an ephemeral volume is lost when the sandbox is deleted. Read [Ephemeral volumes](/Volumes/Ephemeral-volumes) for the full configuration reference, root filesystem mounts, and limits.
