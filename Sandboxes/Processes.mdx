...
<Warning>
  This is an alpha feature that may change or be removed in future. It can only be used with specified versions of the sandbox API and the Blaxel SDKs.
</Warning>
...
- use a specific development version of the Blaxel SDK:

    <CodeGroup>

    ```shell Python (pip)
    pip install blaxel==0.2.63-dev.68
    ```

    ```shell Python (uv)
    uv pip install blaxel=0.2.63-dev.68
    ```
...
await sandbox.process.exec({
  name: "dev-server",
  command: "npm run dev",
  keepAlive: true,        // Prevents auto-hibernation
...
await sandbox.process.exec({
    "name": "dev-server",
    "command": "npm run dev",
    "keep_alive": True,       # Prevents auto-hibernation
    "timeout": 3600           # Auto-kill after 1 hour
})
```
</CodeGroup>
