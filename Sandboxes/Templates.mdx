...
<Note>
  Variables defined here are baked into the image. On the new sandbox generation, variables can also be [updated on a running sandbox](/Sandboxes/Variables-and-secrets#update-variables-on-an-existing-sandbox) or given a new value when you [fork a sandbox](/Sandboxes/Fork#fork-with-new-environment-variables) — both are in preview: [contact us](https://blaxel.ai/contact) or email [support@blaxel.ai](mailto:support@blaxel.ai) to get access. See the [variables and secrets](/Sandboxes/Variables-and-secrets) documentation for details.
</Note>
