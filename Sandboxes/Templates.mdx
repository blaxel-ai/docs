...
Once you have a custom template, you can share it with other workspaces in the same [account](/Security/Workspace-access-control) - for example, to promote a template image from `development` to `production` without rebuilding or re-pushing. Only the metadata record is copied to the target workspace; the underlying storage stays in the source.

When you share a template:

1. The metadata is copied to the target workspace, pointing to the same underlying data in the source.
2. The target workspace can use the shared image to create sandboxes, just like a locally-owned image.
3. Storage billing stays with the source workspace.
4. New tags pushed to the source image are automatically propagated to all workspaces it is shared with.

Shared template in the consuming workspace include a `sourceWorkspace` field. In the [Blaxel Console](https://app.blaxel.ai), they are marked with a badge showing the source workspace name.

### Prerequisites

- Both workspaces must belong to the **same [account](/Security/Workspace-access-control)**. Images cannot be shared between workspaces under different accounts.
- You must be an **admin** of both the source and target workspaces.
- The template must be **locally owned** in the source workspace. You cannot re-share an image that was shared to you from elsewhere.
...
3. Select **Share**.
4. In the dialog, select the target workspace from the dropdown.
...
On the image detail page in the [Blaxel Console](https://app.blaxel.ai), templates shared across workspace are listed with the option to revoke sharing for each workspace individually.
...
You cannot delete a template (or individual tags) while it is shared. You must unshare from all connected workspaces first.

<Warning>
After unsharing, any deployments in the target workspace that reference the shared template will fail on their next restart or scale-up. Make sure no active deployments depend on the template before unsharing.
</Warning>
