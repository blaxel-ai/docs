---
title: "Build reusable sandbox images with Dockerfiles"
sidebarTitle: "Images"
...
| [`blaxel/claude-code:latest`](https://github.com/blaxel-ai/sandbox/tree/main/hub/claude-code) | Claude Code CLI environment with Node.js 24 (slim) |
| [`blaxel/codex:latest`](https://github.com/blaxel-ai/sandbox/tree/main/hub/codex) | Codex CLI environment with Node.js 24 (slim) |
...
<Note>
  Variables set here apply to every sandbox created from this image. On the new sandbox generation, you can also [update variables on a running sandbox](/Sandboxes/Variables-and-secrets#update-variables-on-an-existing-sandbox) or [give them a new value when you fork a sandbox](/Sandboxes/Fork#fork-with-new-environment-variables). See the [variables and secrets](/Sandboxes/Variables-and-secrets) documentation for details.
</Note>
...
const webImage = base
  ...
  .expose(8000)
  .entrypoint("python", "-m", "uvicorn", "main:app", "--host", "::");
...
Refer to the documentation on [sandbox configuration parameters](/Sandboxes/Overview#understand-sandbox-configuration) for more information on the body of the POST request.
...
Everything in a sandbox runs as `root` by default. To scope processes, terminals and filesystem operations to an unprivileged user while the sandbox API keeps the privileges it needs, declare the user in your Dockerfile and enable the feature:
...
See [running sandbox workloads as a non-root user](/Sandboxes/Non-root-user) for the entrypoint variant, what stays privileged, and the limitations.
