...
<Note>
  Variables set here apply to every sandbox created from this image. On the new sandbox generation, you can also [update variables on a running sandbox](/Sandboxes/Variables-and-secrets#update-variables-on-an-existing-sandbox) or [give them a new value when you fork a sandbox](/Sandboxes/Fork#fork-with-new-environment-variables). Both are in preview. See the [variables and secrets](/Sandboxes/Variables-and-secrets) documentation for details.
</Note>
