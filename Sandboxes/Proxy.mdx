---
title: "Proxy & network"
description: "Route outbound sandbox traffic through Blaxel's platform proxy with MITM header/body injection and secrets, configure domain firewalls, and bind egress IPs."
tag: "private preview"
---
...
The Blaxel SDK supports three network features when creating sandboxes: **proxy routing** (MITM header/body injection with secrets), **domain filtering** (allowlist/denylist firewalls), and **egress IP binding**.
...
| `egress` | `EgressConfig` | Dedicated egress IP gateway binding. |
...
| Field | Type | Description |
|---|---|---|
| `gateway` | `string` / `str` | Egress gateway name in the default VPC. |
| `mode` | `string` / `str` | Egress mode (e.g. `"dedicated"`). |
...
| Node.js `https` | HTTPS | Needs manual CONNECT through proxy |
