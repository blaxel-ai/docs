...
<Warning>
  Adding a proxy config to a sandbox enforces the proxy on **all** outbound requests. If some requests should not go through it, use [proxy bypass](./Proxy-secrets-injection#proxy-bypass).
</Warning>
...
| `forbiddenDomains` | `string[]` / `list[str]` | Denylist — all domains except these are reachable. Supports wildcards. If a request matches both lists, `forbiddenDomains` takes precedence and the request is blocked. |
| `firewall` | `FirewallConfig` | Network-level lockdown that forces sandbox egress through the proxy. See [domain filter enforcement](./Proxy-domains#domain-filter-enforcement). |
...
| Field | Type | Description |
|---|---|---|
| `rulesets` | `string[]` / `list[str]` | Lockdown rulesets to apply. Set to `["proxy"]` to force all sandbox outbound traffic through the Blaxel proxy, even for tools that ignore `HTTP_PROXY`/`HTTPS_PROXY`. Can only be set at sandbox creation. |
