...
description: 'Use Blaxel sandbox tools and functions optimized for AI code generation, including file editing, formatting, search, and execution helpers.'
...
Traditional code generation requires generating the entire file every time, which can be slower for large files. With this approach your LLM only generates the specific changes needed, and this tool applies them to the original file.
