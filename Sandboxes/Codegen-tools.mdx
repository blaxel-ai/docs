...
- If using Morph, pass your [Morph API key](https://docs.morphllm.com/api-reference/introduction#authentication) and Morph model (default = *morph-v2*) set as environment variables when creating the sandbox.
- If using Relace, pass your [Relace API key](https://docs.relace.ai/api-reference/introduction#authentication) as an environment variable when creating the sandbox.
...
const sandbox = await SandboxInstance.createIfNotExists({
  ...
  envs: [
    { name: "MORPH_API_KEY", value: process.env.MORPH_API_KEY || "" }, // for Morph
    { name: "MORPH_MODEL", value: process.env.MORPH_MODEL || "morph-v2" } // for Morph
    { name: "RELACE_API_KEY", value: process.env.RELACE_API_KEY || "" } // for Relace
  ]
});
```
```python Python
...
sandbox = await SandboxInstance.create_if_not_exists({
  ...
  "envs": [
    { "name": "MORPH_API_KEY", "value": os.getenv("MORPH_API_KEY") }, # for Morph
    { "name": "MORPH_MODEL", "value": os.getenv("MORPH_MODEL") or "morph-v2" } # for Morph
    { "name": "RELACE_API_KEY", "value": os.getenv("RELACE_API_KEY") }, # for Relace
  ]
})
```
</CodeGroup>

<Note>If you do not pass the MORPH_API_KEY or RELACE_API_KEY, the corresponding tool will not be available.</Note>
