...
This is why a plain `USER` directive is not enough on its own: it de-privileges the sandbox API itself, which breaks drive mounts. Instead, the runtime keeps the sandbox API as root and passes your user down to it, and the API drops privileges for every operation it performs on your behalf.
...
chown -R app "${HOME:-/blaxel}"

exec /usr/local/bin/sandbox-api --user app "$@"
```

The `--user` flag is the explicit opt-in, so it does not need `BL_SANDBOX_USER_ENABLED`. Setting `BL_SANDBOX_USER` in your image environment is equivalent to `USER`, and still requires the enable variable.
