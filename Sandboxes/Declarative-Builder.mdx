...
const sandbox = await image.build({
  ...
  timeout: 900000,             // Timeout in ms (default: 900000 = 15 min)
...
sandbox = await image.build(
    name="my-sandbox",
    memory=8192,                          # Memory in MB (default: 4096)
    timeout=900.0,                        # Timeout in seconds (default: 900)
...
A complete example building a full-stack development sandbox:
...
const sandbox = await ImageInstance.fromRegistry("node:20-bookworm-slim")
  ...
  .npmInstall({ packageManager: "pnpm", globalInstall: true }, "turbo")
  // Python tools
...
async def main():
    ...
    print("Sandbox deployed:", sandbox.metadata.name)
