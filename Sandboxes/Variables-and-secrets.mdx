...
<Note>
  Environment variables cannot be added or changed after a sandbox is created. Set all required variables in your Dockerfile, or at sandbox creation time. To run the same state with a different environment, [fork the sandbox with new variables](/Sandboxes/Fork#fork-with-new-environment-variables).
</Note>
