---
title: 'Snapshots and forking'
sidebarTitle: 'Snapshots & forking'
description: 'Create point-in-time snapshots of sandboxes, roll them back to a previous checkpoint, and fork them into new sandboxes.'
tag: "private preview"
---

<Note>
  This feature is currently in private preview and is not recommended for production use.
</Note>

Snapshots capture the current state of a sandbox at a point in time. Use snapshots to checkpoint sandbox state to allow rollbacks after making changes, or to fork a new sandbox from a checkpoint you picked. Forking a sandbox into another sandbox does not require a snapshot: it copies the source's live state directly.

## Create a snapshot

Create a snapshot of an existing sandbox. The snapshot captures the filesystem, running processes, and memory state.

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
const snapshot = await sandbox.snapshot("before-migration");
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
snapshot = await sandbox.snapshot("before-migration")
```

```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/sandboxes/my-sandbox/snapshots \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace" \
  -H "Content-Type: application/json" \
  -d '{ "name": "before-migration" }'
```

</CodeGroup>


## List snapshots

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
const snapshots = await sandbox.listSnapshots();
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
snapshots = await sandbox.list_snapshots()
```

```bash HTTP API
curl https://api.blaxel.ai/v0/sandboxes/my-sandbox/snapshots \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace"
```

</CodeGroup>

## Delete a snapshot

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
await sandbox.deleteSnapshot("snap_abc123");
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
await sandbox.delete_snapshot("snap_abc123")
```

```bash HTTP API
curl -X DELETE https://api.blaxel.ai/v0/sandboxes/my-sandbox/snapshots/snap_abc123 \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace"
```

</CodeGroup>


## Restore a sandbox to a snapshot

Restoring rolls a sandbox back to one of its own snapshots, in place. The sandbox keeps its name, its URLs, its previews, and its configuration: only the instance behind it is torn down and rebuilt from the snapshot, on the same machine the snapshot was taken on.

<Warning>
  Everything the sandbox wrote after the snapshot was taken is lost: files, running processes, and memory state. Take a new snapshot first if you want to be able to come back to the current state.
</Warning>

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
await sandbox.restore("snap_abc123");
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
await sandbox.restore("snap_abc123")
```

```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/sandboxes/my-sandbox/snapshots/snap_abc123/restore \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace"
```

</CodeGroup>

The restore returns as soon as it is accepted, without waiting for the sandbox to be back up. The sandbox answers again once its instance has been rebuilt, so retry your first calls while it resumes.

Only a snapshot of that same sandbox can be restored, and only once it is `ready`. To roll back onto a *different* sandbox, [fork from the snapshot](#fork-from-a-snapshot) instead.

You can also restore from the Blaxel console: open your sandbox, go to the **Snapshots** tab, and use the **Restore** action on the snapshot you want to go back to.

## Fork a sandbox

Forking creates a new sandbox from an existing one, forking its entire memory state (files + running processes). The fork inherits the source sandbox's image, memory configuration, environment variables, and port settings.

Calling `fork()` copies the source sandbox's live state straight into the new sandbox. No snapshot is created, persisted, or listed under the source sandbox.

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
const result = await sandbox.fork("my-sandbox-copy");
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
result = await sandbox.fork("my-sandbox-copy")
```

```bash CLI
bl fork sbx/my-sandbox sbx/my-sandbox-copy
```

```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/sandboxes/my-sandbox/fork \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace" \
  -H "Content-Type: application/json" \
  -d '{
    "targetType": "sandbox",
    "targetName": "my-sandbox-copy"
  }'
```

</CodeGroup>

A fork lands on the same node as its source. Forking a running sandbox pauses it for the moment its memory is captured, then both sandboxes run on. A sandbox in standby is not woken to be forked.

Forking into an application (`targetType: "application"`) still goes through a snapshot, because the application revision references that snapshot durably so you can re-activate the revision later. See [create an application from a sandbox fork](/Applications/Overview#create-an-application-from-a-sandbox-fork).

### Fork from a snapshot

To fork from a checkpoint instead of the source's live state, pass the `snapshotId` of a snapshot you took earlier. The fork is created from that snapshot, and nothing is captured from the source, so a running source is not paused:

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
const result = await sandbox.fork("my-sandbox-copy", {
  targetType: "sandbox",
  snapshotId: "snap_abc123",
});
```

```python Python
from blaxel.core import SandboxInstance

sandbox = await SandboxInstance.get("my-sandbox")
result = await sandbox.fork(
    "my-sandbox-copy",
    target_type="sandbox",
    snapshot_id="snap_abc123",
)
```

```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/sandboxes/my-sandbox/fork \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace" \
  -H "Content-Type: application/json" \
  -d '{
    "targetType": "sandbox",
    "targetName": "my-sandbox-copy",
    "snapshotId": "snap_abc123"
  }'
```

</CodeGroup>


### Fork with new environment variables

Pass `envs` to give the fork a different environment from its source. Each variable the source already carries takes the value you pass, each variable it does not carry is added, and every other variable of the source is inherited. The source sandbox is left untouched.

<CodeGroup>

```typescript TypeScript
import { SandboxInstance } from "@blaxel/core";

const sandbox = await SandboxInstance.get("my-sandbox");
const result = await sandbox.fork("my-sandbox-copy", {
  envs: [
    { name: "NODE_ENV", value: "staging" },
    { name: "FEATURE_FLAG", value: "1" },
  ],
});
```

```python Python
from blaxel.core import SandboxInstance
from blaxel.core.client.models.env import Env

sandbox = await SandboxInstance.get("my-sandbox")
result = await sandbox.fork(
    "my-sandbox-copy",
    envs=[
        Env(name="NODE_ENV", value="staging"),
        Env(name="FEATURE_FLAG", value="1"),
    ],
)
```

```bash HTTP API
curl -X POST https://api.blaxel.ai/v0/sandboxes/my-sandbox/fork \
  -H "Authorization: Bearer $BL_API_KEY" \
  -H "X-Blaxel-Workspace: my-workspace" \
  -H "Content-Type: application/json" \
  -d '{
    "targetType": "sandbox",
    "targetName": "my-sandbox-copy",
    "envs": [
      { "name": "NODE_ENV", "value": "staging" },
      { "name": "FEATURE_FLAG", "value": "1" }
    ]
  }'
```

</CodeGroup>

The fork starts with the new environment already applied, so processes you launch in it read the new values. Processes that were already running in the source when its state was captured keep the environment they were started with, as an environment change cannot reach a process that is already running.

<Note>
  `envs` only sets and adds variables. A variable of the source cannot be removed through a fork.
</Note>

`envs` works the same way when forking from a snapshot.

## Fork request parameters

| Parameter | Type | Description |
|-----------|------|-------------|
| `targetType` | string | Target resource type: `sandbox` |
| `targetName` | string | Name of the sandbox to create |
| `port` | integer | Port to expose |
| `snapshotId` | string | Fork from a snapshot you took earlier, instead of from the source's live state |
| `envs` | array | Environment variables the fork runs with, on top of the ones inherited from the source |

## Fork response

| Field | Type | Description |
|-------|------|-------------|
| `name` | string | Name of the created sandbox or application |
| `type` | string | What was created: `sandbox` or `application` |
| `snapshotId` | string | The snapshot the fork was created from. Set only when the fork went through a snapshot — you passed a `snapshotId`, or you forked into an application — and empty for a direct sandbox fork |

<CardGroup cols={1}>
  <Card title="Sandbox overview" icon="cube" href="/Sandboxes/Overview">
    Learn more about sandbox lifecycle and configuration.
  </Card>
</CardGroup>
