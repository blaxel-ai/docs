...
title: 'Blaxel: infrastructure for autonomous agents'

description: 'Introduction to Blaxel core building blocks: perpetual sandboxes, Agent Drive shared filesystem, and networking primitives.'

sidebarTitle: "Overview"

---

Blaxel is the **infrastructure for autonomous agents**. Our platform lets you keep infinite, secure sandboxes on automatic standby while co-hosting your agents for near instant latency.
...
Blaxel is the infrastructure for autonomous agents. It **doesn't force you into any kind of workflow** or shaped box. While we encourage you to exploit architecture designs that we consider are more reliable, our toolkit gives you all the pieces you need to build reliable agentic systems exactly the way you want.
...
At the heart of Blaxel is our flagship **perpetual sandbox** service. Sandboxes are secure, instant-launching compute environments that you can use for running AI code. Blaxel lets you keep sandboxes on automatic standby with optimized agent hosting for near instant latency.
...
Choose storage based on durability and sharing needs:

- [**Sandbox filesystem**](/Sandboxes/Overview#memory-and-filesystem): Fastest option (in-memory, no network or disk I/O). Use it for scratch files, working directories, and outputs that only need to live for the current sandbox. It survives standby and resume, but is erased when the sandbox is deleted. Not replicated or durable; capacity is roughly half the sandbox memory allocation.
- [**Volumes**](/Volumes/Overview): Durable block storage that survives sandbox deletion and attaches to one sandbox at a time. Use it for high-performance persistence in a single sandbox lineage (databases, source checkouts, agent memories). Mounts only at sandbox creation; re-attaching to a new sandbox costs a cold boot of a few hundred ms.
- [**Agent Drive**](/Agent-drive/Overview): Shared distributed filesystem that multiple sandboxes or agents can mount and write concurrently. Use it to share state across sandboxes, sessions, or tenants. Attach or detach on running sandboxes at any path, with fine-grained permissions. Prefer a Volume for single-writer block workloads like databases.

As a rule of thumb: if the data only needs to survive the current sandbox session, use the sandbox filesystem. If it needs to outlive the sandbox but only one sandbox will ever touch it at a time, use a Volume. If more than one sandbox or agent needs to read or write it at the same time, use Agent Drive. Because the sandbox filesystem is not replicated or durable, if the data is not easily recreated, use a Volume or Agent Drive instead of relying on it alone.

For a deeper comparison, see [Choose the right storage for your Blaxel agents](https://blaxel.ai/blog/choose-the-right-storage-for-your-blaxel-agents).
...
- [**Agent Skill & MCP:**](skills-mcp) Let your coding agent (Cursor, Claude Code, Windsurf…) create and manage Blaxel resources using natural language.
- **Product Documentation**
    ...
    - [**Agent Drive**](Agent-drive/Overview): Share files across sandboxes and agents with a distributed filesystem.
    - [**Volumes**](Volumes/Overview): Persist files long-term by attaching volumes to your resources.
    - [**Integrations**](Integrations): Discover how Blaxel works with other tools, frameworks, and platforms.
    - [**Observability**](Observability/Overview): Monitor logs, traces and metrics for your agent runs.
    - [**Governance**](Model-Governance/Policies): Manage your AI deployment strategies with policies and environments.
    ...
    - [**Troubleshooting**](troubleshooting): Resolve common issues with Blaxel deployments.
- **Tutorials & Examples**
    - [**Sandbox tutorials**](Tutorials/Sandboxes-Overview): Run web applications in sandboxes with Astro, Expo, and Next.js.
    - [**Agent tutorials**](Tutorials/Agents-Overview): Deploy agents using popular frameworks like Claude Agent SDK, LangChain, CrewAI, and more.
- **References**
    - [**API reference**](api-reference/introduction): Comprehensive guide to Blaxel's APIs.
    - [**SDK reference**](sdk-reference/introduction): Manage Blaxel resources programmatically using TypeScript, Python, or Go SDKs.
    - [**CLI reference**](cli-reference/introduction): Learn how to use Blaxel's command-line interface.
    - [**Deployment reference**](deployment-reference): Complete reference for the blaxel.toml configuration file.
