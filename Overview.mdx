...
description: 'Blaxel is a perpetual sandbox platform for AI agents. Explore docs for cloud sandboxes, agent hosting, MCP servers, batch jobs, model gateway, and more.'
...
Install the [Blaxel skill](./connect-ai) to give your agent the ability to deploy AI agents, create sandboxes for code execution, host MCP servers, run batch jobs, and more...all using simple prompts, zero code required!
...
Blaxel is a cloud where **AI agents themselves are the primary users**. All products are accessible through [MCP servers](connect-ai), allowing agents to create and manage resources via tool calls. Blaxel provides agents with all the compute they need to scale and perform optimally: products like Sandboxes give them their own dedicated personal computer(s) / computing environments, while Batch Jobs enable them to schedule background tasks at scale.
...
- [**Agent Skill & MCP:**](connect-ai) Let your coding agent (Cursor, Claude Code, Windsurf…) create and manage Blaxel resources using natural language.
