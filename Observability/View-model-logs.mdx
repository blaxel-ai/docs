...
Depending on [how you are deployed on Global Inference Network](../Models/Global-Inference-Network.mdx), logs storage and querying will behave differently.

- Models that are deployed on Beamlit, either by [direct deployment](../Models/Model-deployment.mdx) or when [overflowing on Beamlit](../Models/Cloud-Burst-Network/Model-overflow.mdx), **automatically export inference logs**. They can be [viewed](View-model-logs.mdx) per specific model deployment.
- Models that are deployed on your own clusters as an [origin to be offloaded on Beamlit](../Models/Cloud-Burst-Network/Model-overflow.mdx) **do not export logs**. Please [contact us](mailto:support@beamlit.com) if you would be interested to centralize these logs with Beamlit logs, either on Beamlit or in your cluster.
