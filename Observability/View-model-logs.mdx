...
Depending on [how you are deployed on Global Inference Network](../Models/Global-Inference-Network.md), logs storage and querying will behave differently.

- Models that are deployed on Beamlit, either by [direct deployment](../Models/Model-deployment.md) or when [overflowing on Beamlit](../Models/Cloud-Burst-Network/Model-overflow.md), **automatically export inference logs**. They can be [viewed](View-model-logs.md) per specific model deployment.
- Models that are deployed on your own clusters as an [origin to be offloaded on Beamlit](../Models/Cloud-Burst-Network/Model-overflow.md) **do not export logs**. Please [contact us](mailto:support@beamlit.com) if you would be interested to centralize these logs with Beamlit logs, either on Beamlit or in your cluster.
