...
<Tip>You could also just bring your own code and add [Beamlit SDK commands and decorators](Agents/Develop-an-agent) to deploy it. This scaffolded repository gives you everything set up and ready to go from scratch.</Tip>
...
To push to Beamlit on the default *production* environment, run the following command. You’ll need to [have *Docker* installed](https://docs.docker.com/get-started/get-docker/) on your machine to build the app.
...
You can also run inference requests on your agent (or each function or model API) from the Beamlit console by using the workbench.
