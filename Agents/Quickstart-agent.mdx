...
You can deploy agents developed with any framework or SDK on Blaxel, so long as the agent exposes an HTTP endpoint for requests. This quickstart uses the [Claude Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview), but you could also use other frameworks like LangChain, Vercel AI SDK, Mastra, or your own custom code.
...
app.post("/query", async (req, res) => {
  const { prompt } = req.body as QueryRequest;
