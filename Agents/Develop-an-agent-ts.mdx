...
- **agent.ts (or src/agent.ts)**: This file must contain an *async* function that runs your agent. This function serves as the default entry point during serving and deployment.
    ...
    - The function accepts these arguments, which Beamlit passes when a request is made to your deployed agent:
        ...
        - *args*: (Optional) The core agent algorithm. By default, it uses a [LangChain ReAct agent](https://v03.api.js.langchain.com/functions/langchain.agents.createReactAgent.html).
...
You can use model APIs deployed on Beamlit by calling `getChatModel()`. This function returns a model API **in LangChain format [*ChatModel*](https://js.langchain.com/docs/integrations/chat/)**
...
You can use [functions](../Functions/Overview) defined both in your code and deployed on Beamlit by calling `getFunctions()`. This function returns a list of tools **in LangChain format** [BaseToolkit](https://v03.api.js.langchain.com/classes/_langchain_core.tools.BaseToolkit.html)**.**
