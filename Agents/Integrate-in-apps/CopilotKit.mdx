...
Make sure to replace `https://run.blaxel.ai/{YOUR-WORKSPACE}/agents/{YOUR-AGENT}/copilotkit` and `<API_KEY / TOKEN>` .

```typescript app/api/copilotkit/route.ts {13,23}
import {
  CopilotRuntime,
  ExperimentalEmptyAdapter,
  copilotRuntimeNextJSAppRouterEndpoint,
...
<Warning>Make sure to call the `/copilot` endpoint on your agent.</Warning>