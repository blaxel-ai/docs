...
You can provide default (fallback) values for both secret references and environment variable references using a `:` separator inside the interpolation syntax. If the variable is not found in the environment or in loaded secrets, the default value is used instead.

```toml blaxel.toml
[env]
# Secret with a default value
API_URL = "${secrets.API_URL:https://api.example.com}"

# Environment variable with a default value
BL_REGION = "${BL_REGION:us-pdx-1}"
```
...
| Pattern | Description |
|---------|-------------|
| `${secrets.KEY:default}` | Resolves from env or secrets, falls back to `default` |
| `${KEY:default}` | Resolves from env, falls back to `default` |

<Tip>Default values let you share a single `blaxel.toml` across environments. Set environment-specific values through `.env` files or shell variables, and rely on defaults for everything else.</Tip>
