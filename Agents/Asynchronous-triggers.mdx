...
Asynchronous triggers let your agent perform requests asynchronously and, optionally, notify you when complete via callbacks. With this approach, the agent returns immediately while the task continues running in the background. This is useful for long-running operations, workflows, or when your endpoint must respond quickly.
...
You can provide a callback URL to receive the result when the background job completes. The callback URL must be specified when the agent is deployed, either via the [Blaxel dashboard](#blaxel-dashboard) or the [`blaxel.toml` configuration file](#blaxel-configuration-file).
...
Asynchronous triggers are configured in two ways: via the Blaxel dashboard or in the `blaxel.toml` configuration file.

### Blaxel dashboard

In the Blaxel dashboard, you can configure asynchronous triggers by clicking the **Triggers** tab on the agent detail page. This page lets you list, add, edit, or delete triggers for your agent.
