...
description: 'Build, test, and run a custom Python agent locally with the Blaxel SDK and CLI, with code examples for LangChain, CrewAI, and other frameworks.'
...
Agents Hosting has few requirements or limitations:
...
You can develop agents by **mixing tools defined locally in your agents, and tools defined as remote servers**. Using separated tools prevents monolithic designs which makes maintenance easier in the long run. Let's look at a practical example combining remote and local tools. The code below uses two tools:
...
This `entrypoint` section is optional. If not specified, Blaxel will automatically detect the agent’s content and configure your agent startup settings.
