...
- policies on **locations** allow to pass a specific list of locations (which represent physical execution clusters) on which the workload can be deployed and executed
    - for example, execute only in the following locations: *San Francisco, USA* and *Tokyo, Japan*
...
When no policy on a certain type is applied to an environment or a model deployment, **all options for this type are considered allowed**. Workloads are executed using Global Inference Network’s default optimizations.
...
WHAT IF COMBINING ENVIRONMENT AND DEPLOYMENT → MANDATORY VS OPTIONAL (reusable block?)

For example:

- Let’s assume the following policies:
    - Policy A: Country is USA
    - Policy B: Location is in: San Francisco, USA or Tokyo, Japan
    - Policy C: GPU is NVIDIA T4
- if the environment has the following combined policies:
    - A and B: then workloads will only execute in Tokyo, Japan, and in any location in the USA (including San Francisco) — on any kind of GPU available there.
    ...
    - A and B and C: then workloads will only execute in Tokyo, Japan, and in any location in the USA (including San Francisco) — only on T4 GPUs.
...
HOW TO SPECIFY MANDATORY VS ENFORCE
...
refer to section ‘multiple policies’ from here
...
Read about the available parameters in the location reference.