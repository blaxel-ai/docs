...
description: 'Define policies to control where and how workloads execute on the Global Agentics Network, including location constraints, flavor selection, and token usage limits.'
...
- policies on **countries** allow to define all [physical locations](/Model-Governance/Policies) inside of one or several countries at once
    - for example, execute only in the following country: *USA*
- policies on **continents** allow to define all [physical locations](/Model-Governance/Policies) inside of one or several continents at once
