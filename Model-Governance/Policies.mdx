...
Policies are used to program how and where your workloads are deployed on Beamlit. Policies can be defined as code, allowing for easy programming and customization of your [Global Inference Network](../Models/Global-Inference-Network.mdx).

Policies apply to the entities they are attached to. They can be attached to individual [model deployments](../Models/Model-deployment.mdx), or to a whole [environment](Environments.mdx) to enforce them across your workspace.
...
When no policies are enforced on a type, all options for this type are considered allowed. Workloads are executed using [Global Inference Network](../Models/Global-Inference-Network.mdx)’s default optimizations.
...
- policies on **countries** allow to define all [physical locations](Policies.mdx) inside of one or several countries at once
    - for example, execute only in the following country: *USA*
- policies on **continents** allow to define all [physical locations](Policies.mdx) inside of one or several continents at once
    - for example, execute only in *North America*

Refer to [our reference on policy locations](Policies.mdx) below for more information.
...
Refer to [our reference on policy flavors](Policies.mdx) below for more information.
...
Attaching a policy to a workload enforces it on the workload. Policies can be attached either to [**environments**](Environments.mdx), or to specific [**model deployments**](../Models/Model-deployment.mdx) (provided they [do not collide](Policies.mdx) with the environment-level policies).
...
In this situation, their combined effect is the **UNION** of all of their effects for the same [type](Policies.mdx) of policy (a.k.a *OR* clause), and **INTERSECTION** across all [types](Policies.mdx) of policies (a.k.a *AND* clause).

**If you are attaching policies to a workload directly that already inherits policies from a linked environment**

In this situation, a user cannot allow more options **on a policy [type](Policies.mdx)** **regulated by environment-level policies** by using deployment-specific policies. In other words, the combined effect is the **INTERSECTION** of all of their effects for the same type.
...
Attaching a policy to an environment **enforces it on all workloads** that are [deployed on this environment](Environments.mdx), and that are in the scope of the [target](Policies.mdx) of the policy.
...
Attaching a policy to a model deployment directly **enforces it on this specific model deployment,** provided it is in the scope of the [target](Policies.mdx) of the policy.

If the deployment is already inheriting policies from its environment, the resulting effect is [as described above](Policies.mdx). You will not be able to allow more options than what is restricted at environment level.
