...
Policies apply to the entities they are attached to. They can be attached to individual model API/function/agent deployments, or to a whole [environment](Environments) to enforce them across your workspace.
...
Targets specify the resource types the policy should influence. For instance, if you want to enforce the use of specific GPUs in the *production* environment for agents, but allow functions to run on CPU, you can define a target on *agents* for that policy.
...
Attaching a policy to a workload enforces it on the workload. Policies can be attached either to [**environments**](Environments%2013847e47b1ea81ac9677d81f847741bd), or to specific model API/function/agent deployments (provided they [do not collide](Policies) with the environment-level policies).

### Attaching multiple policies

When attaching **multiple policies** to a resource, it's crucial to understand their combined effect. This effect varies depending on whether you're attaching multiple policies to the same workload (e.g., one environment, or one deployment), or applying policies to a workload that already inherits policies from its linked environment. 
...
### Attaching a policy to a deployment

Attaching a policy to a workload directly **enforces it on this specific deployment,** provided it is in the scope of the [target](Policies) of the policy.
