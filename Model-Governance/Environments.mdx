...
- They are created without any [policy](Policies.md): by default, models deployed in these environments won’t have any deployment restrictions.
...
You can attach [deployment policies](Policies.md) to any environment in order to enforce them across all workloads in that environment that are in the scope of the [target](Policies.md) of the policy.
...
When [deploying a model](../Models/Model-deployment.md), you can choose an environment on which to deploy it. When a model is deployed in an environment, this produces a “*model deployment*”.
...
1. Let’s say you have version A of the model running on the *production* environment (this is the  production deployment), and version B of the model running on the *development* environment (this is the development deployment). Each version can be called through the [respective model deployment endpoint](../Models/Query-a-model.md) allowing to test version B while version A still runs.
2. Once version B is tested and ready, you [release](../Models/Model-deployment.md) the *development deployment* on the *production* environment.
...
4. You [replace](../Models/Model-deployment.md) the *development* deployment with version C of the model. 
