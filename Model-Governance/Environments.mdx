...
description: 'Define environments to enforce consistent deployment strategies, isolation, and promotion workflows across your entire Blaxel workspace.'
...
For example, let’s say you attach the following two policies on environment ‘development’:
...
1. Let’s say you have version A of the agent running on the *production* environment (this is the production deployment), and version B of the agent running on the *development* environment (this is the development deployment). Each version can be called through the [respective agent deployment endpoint](../Agents/Query-agents) allowing you to test version B while version A still runs.
