...
There is one distinct endpoint for each **model deployment,** i.e. for each combination of a [model](../Models/Model-deployment.mdx) and an [environment](../Model-Governance/Environments.mdx) on which it is deployed. 
