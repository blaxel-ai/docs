...
Note that this code references a custom tool from another folder: `customfuncs.helloworld.helloworld`. By default, this function **will not** be deployed as a separate component. See below for instructions on deploying it in its own sandboxed environment.

Make sure you have a function that handles an execution of the agent: it will serve as the default entry point during serving and deployment.
...
Here’s an example with the Beamlit decorator:
