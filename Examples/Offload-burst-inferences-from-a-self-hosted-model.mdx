...
On a conceptual level, model offloading works by defining a [model deployment](../Models/Model-deployment.md) on Beamlit that references a Kubernetes Deployment on your own cluster. The is done **via the open-source [Beamlit controller](https://github.com/beamlit/beamlit-controller).** You can interact with the controller using Kubernetes *Custom Resources (CR)*, in order to:
...
[Service accounts](../Security/Service-accounts.md) act as virtual users in your [Beamlit workspace](../Security/Workspace-access-control.md), representing external systems that need to control Beamlit resources. The Beamlit controller authenticates using a service account in your workspace.
...
With this resource, the Beamlit controller will monitor a Prometheus server for the value of the offloading metric. If you want to use Kubernetes’ metrics-server instead, follow [our guide here](../Models/Cloud-Burst-Network/Cluster-configurations/Set-up-overflow-metric.md).
