...
[Ephemeral volumes](/Volumes/Ephemeral-volumes) are temporary disk-backed storage attached to a job at startup and automatically destroyed when the job completes. Ephemeral volumes have separate quotas from persistent volumes. Limits depend on your [tier](app.blaxel.ai/account/quotas).
