...
The offloading metric is a customizable infrastructure metric used by the Beamlit controller to trigger [model offloading](../Model-overflow.mdx) when it hits a certain threshold.
...
- Set up the [Beamlit controller](../Install-Beamlit-operator.mdx) to monitor your Prometheus service by adding the following configuration in the controller chart’s [*values.yaml*](https://github.com/beamlit/beamlit-controller/blob/main/chart/values.yaml):
