...
Make a **POST** request to the [inference endpoint](Query-a-model.md) for the model deployment you are requesting, making sure to fill in the [authentication token](Query-a-model.md):
...
You can call another deployment with the option `--env` , and you can call [specific API endpoints](Query-a-model.md) that your model implements with the option `--path` :
