...
Model [deployments](Model-deployment.mdx) on Beamlit have an **inference endpoint** which can be used by external consumers to request an inference execution. Inference requests are then routed on the [Global Inference Network](Global-Inference-Network.mdx) based on the [deployment policies](Model-deployment.mdx) associated with your model deployment. 
...
There is one distinct endpoint for each **model deployment,** i.e. for each combination of a [model](Model-deployment.mdx) and an [environment](../Model-Governance/Environments.mdx) on which it is deployed. 
...
By default, models deployed on Beamlit aren’t public. It is necessary to authenticate all inference requests, via a [bearer token](../Security/Access-tokens.mdx).

The evaluation of authentication/authorization for inference requests is managed by the Global Inference Network based on the [access given in your workspace](../Security/Workspace-access-control.mdx).
...
Make a **POST** request to the [inference endpoint](Query-a-model.mdx) for the model deployment you are requesting, making sure to fill in the [authentication token](Query-a-model.mdx):
...
You can call another deployment with the option `--env` , and you can call [specific API endpoints](Query-a-model.mdx) that your model implements with the option `--path` :

```bash
beamlit run your-model --env development --path /v1/chat/completions --data '{"inputs":"Hello there!"}' 
```
