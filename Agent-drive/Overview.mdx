...
<Warning>
  Drive permission rules (labels, `path`, `mode`) are **not currently enforced** for S3 access. A valid service-account API key grants full read-write access to the entire drive over S3, regardless of any [drive permissions](/Agent-drive/Permissions) configured on it. Only issue S3 credentials to service accounts you're comfortable giving full access to the drive; enforcing permission rules over S3 is planned but not yet available.
</Warning>
...
print(drive.drive.state.s_3_url)  # low-level field name; a higher-level property is planned
```
