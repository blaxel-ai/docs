...
When the sandbox process runs as a non-root user, files on the drive (owned by UID/GID `0` on the filer) may appear inaccessible. You can remap file ownership at mount time by specifying a local UID and/or GID. The filer side is always `0`.
...
Files owned by UID/GID `0` on the filer will appear as owned by UID/GID `1000` inside the sandbox. Both fields are optional and accept any non-negative integer.
...
| Variable | Description |
|---|---|
| `BLFS_UID_MAP` | Default local UID to map to filer UID `0` |
| `BLFS_GID_MAP` | Default local GID to map to filer GID `0` |
